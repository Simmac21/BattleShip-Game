...
public class SampleBot {
    ...
    private ArrayList<Integer> battleShipLengthArray = new ArrayList<Integer>();
    private ArrayList<Integer> battleShipCharArray = new ArrayList<Integer>();
    ...
    public boolean fireShot() {
        ...
        while (availableMissiles != 0 && hits != 17) {
            ...
            System.out.println("To Attack choose your coordinates. Enter the row you wish to attack. (Ex: A, B, C): ");
            ...
            for (int j = 0; j < battleShipLengthArray.size(); j++) {
                // checks to see if it was a hit or a miss
                for (int i = 0; i < battleShipLengthArray.get(j); i++) {
                    if (gameBoard[coord1][coord2] == getLetter(battleShipCharArray.get(j))) {
    ...
    public void difficultyInfo() {
        ...
        while (difficultyLevel != 1 && difficultyLevel != 2 && difficultyLevel != 3) {
            ...
            if (difficultyLevel == 0) {
                ...
                System.out.print("\n" + "For Easy Enter 1,for standard 2 and for advance 3 ");
    ...
    public void createShipArray() {
        ...
        battleShipLengthArray.add(ships.AIRCRAFT.getShipLength());
        battleShipLengthArray.add(ships.BATTLESHIP.getShipLength());
        battleShipLengthArray.add(ships.DESTROYER.getShipLength());
        battleShipLengthArray.add(ships.SUBMARINE.getShipLength());
        battleShipLengthArray.add(ships.PATROL.getShipLength());

        // stores each ship marker from enum
        battleShipCharArray.add(ships.AIRCRAFT.getShipMarker());
        battleShipCharArray.add(ships.BATTLESHIP.getShipMarker());
        battleShipCharArray.add(ships.DESTROYER.getShipMarker());
        battleShipCharArray.add(ships.SUBMARINE.getShipMarker());
        battleShipCharArray.add(ships.PATROL.getShipMarker());
    ...
    public void positionShips() {
        ...
        for (int j = 0; j < battleShipLengthArray.size(); j++) {
            ...
            while (!shipsOnBoard) {
                ...
                if (horizontal) {
                    ...
                    for (int i = 0; i < battleShipLengthArray.get(j); i++) {
                        ...
                        if (gameBoard[row][i + 1] != 0) {
                            ...
                            gameBoard[row][i + 1] = getLetter(battleShipCharArray.get(j));
                        }
                    }
                    ...
                    for (int i = 0; i < battleShipLengthArray.get(j); i++) {
                        ...
                        if (gameBoard[i + 1][col] != 0) {
                            ...
                            gameBoard[i + 1][col] = getLetter(battleShipCharArray.get(j));
                        }
                    }
                ...
                //}// end space free
            } // end if else statement // end if else statement // end if else statement // end if else statement
